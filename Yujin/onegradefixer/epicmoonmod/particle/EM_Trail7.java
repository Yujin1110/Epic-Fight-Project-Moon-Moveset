...
public class EM_Trail7 extends AnimationTrailParticle {
    ...
    protected void createNextCurve() {
        ...
        if(isTrailInvisible)  {}
            ...
            AAALevel.addParticle(level, true, info.position(prevStartPos).position(prevEndPos).rotation(180F, - (float) Math.toRadians(this.owner.getOriginal().getYRot()),0F).clone());
        }
    }
