...
public class EM_Trail5 extends AnimationTrailParticle {
    ...
    protected void createNextCurve() {
        ...
        if(isTrailInvisible)  {}
            ...
            AAALevel.addParticle(level, true, info.position(prevStartPos).position(prevEndPos).rotation(-360F, - (float) Math.toRadians(this.owner.getOriginal().getYRot()),0F).clone());
        }
    }
